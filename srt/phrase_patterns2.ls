...
aggregate_sentence_patterns = (tree) ->
  ...
  target_idx = parsed.childrentext.indexOf target_phrase
  if target_idx != -1
    ...
    output.push {target_idx, pos: parsed.pos, pattern: pattern, childrentext: parsed.childrentext}
...
for pattern in all_patterns
  ...
  for pattern_counts_for_pos,pos_idx in usage_info[pattern].patterncounts
    target_idx = usage_info[pattern].target_idx
